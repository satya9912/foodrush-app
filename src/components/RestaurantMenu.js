...
function RestaurantMenu() {
  ...
  return (
    ...
    <div className=''>
      <div className='bg-[#131921]  p-3 rounded-md  mt-20 mb-3 border-2 flex justify-center align-top'>
        <div className='mr-3'>
          <img className=' rounded-md object-contain w-60' src={IMAGE_CDN_URL + cloudinaryImageId} />
        </div>
        <div>
        ...
        {menuItems.map( (item, index)=> (
          ...
          <div className='flex w-[900px] justify-between align-middle mx-80 mb-5 border-2 p-4'>
            <div className='w-2/3'>
              ...
              <p className='mt-1'>₹ {item.price ? item.price/100 : 
                item.defaultPrice/100}</p>
              <p className='text-gray-400 mt-1'>{item.description}</p>  
              <div className='flex'>
              <span className='mr-[1px] mt-1'>{item.ratings.aggregatedRating.rating ? item.ratings.aggregatedRating.rating: 3.9}</span>
